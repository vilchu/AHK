...
^+c:: {
    ClipSaved := ClipboardAll()   ; Save full clipboard (text/images/etc.)
    A_Clipboard := ""             ; Clear so we can detect "no selection"
    Send "^c"
    ClipWait 0.5

    query := A_Clipboard ? UriEncode(A_Clipboard) : ""
...
; - If text is selected: search it
; - If not: open YouTube home

^+y:: {
    ClipSaved := ClipboardAll()   ; Save full clipboard
    A_Clipboard := ""             ; Clear to detect selection
    Send "^c"
...
;; Hotstring syntax: :<options>:<trigger_string>::<replacement>
...
;; These two need to be defined before the `h;` → `ö` hotstring or it will take precedence
:*?b0:dash;::
{
;; Do nothing so that typing the HTML entities `&ndash;` and `&mdash;` won't produce `&ndasö` and `&mdasö`
}
:*?b0:length;::
{
;; Again, do nothing so that typing e.g. `foo.length;` won't produce `foo.lengtö`
}

;; No need to define separate hotstrings for uppercase letters because hotstrings are "case-conforming" by default. Applies to the `;ao` → `å` hotstring as well.
